...
async function getPendingPreparedNpmReleaseState( config, deps = {} ) {
	...
	if (
		packageTags.every(
			( tagName ) => remoteTagShas.get( tagName ) === publishCommit
		)
	) {
		return null;
	}
...
async function finalizePreparedNpmRelease( config, deps = {} ) {
	...
	const conflictingTag = packageTags.find( ( tagName ) => {
		const remoteSha = remoteTagShas.get( tagName );
		return remoteSha && remoteSha !== releaseState.publishCommit;
	} );
	if ( conflictingTag ) {
		throw new Error(
			`Package tag ${ conflictingTag } points to ${ remoteTagShas.get(
				conflictingTag
			) }, expected ${ releaseState.publishCommit }.`
		);
	}
	if (
		packageTags.every(
			( tagName ) =>
				remoteTagShas.get( tagName ) === releaseState.publishCommit
		)
	) {
	...
	const missingPackageTags = packageTags.filter(
		( tagName ) => ! remoteTagShas.has( tagName )
	);
...
async function runPackagesRelease( config, customMessages ) {
	...
	if ( [ 'prepare', 'all' ].includes( config.phase ) ) {
		hasPreparedRelease = Boolean( await prepareNpmRelease( config ) );
	...
	if ( hasPreparedRelease && [ 'publish', 'all' ].includes( config.phase ) ) {
		await publishPreparedPackagesToNpm( config );
	}
	if (
		hasPreparedRelease &&
		[ 'finalize', 'all' ].includes( config.phase )
	) {
		await finalizePreparedNpmRelease( config );
	}
...
function getConfig(
	releaseType,
	{ ci, phase = 'all', repositoryPath, semver = 'patch', wpVersion }
) {
